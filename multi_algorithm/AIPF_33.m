function pos = IPF_33(init_x,init_y,sl1,deg1,sigma1,sigma2,mag1,dataset,fx,xfx)
%%%%%%%%初始化操作%%%%%%%%%%%%%%%%%%%%%%
v_l=0;                  %初始速度
v_d=0;
...
%%%%%%%%%迭代处理每一步的位置和方向数据，同时更新速度估计，最终得到存储每一步位置信息的pos矩阵%%%%%%%%%%%%
    for s=1:step
            ...
            row=ceil((sl+3*sigma1)*6*sigma2/pp);%分别计算用于位置估计的网格的行和列数
            ...
            for m=1:10
                for i=1:row
                    for j=1:col
                        %分别计算当前网格单元的行边界row_l,row_s和列边界col_1,col_s
                        ...
                        index=Find_33(dataset,x_t,y_t);%根据更新后的位置坐标在dataset中查找相关数据点的索引
                        ...
                        if(index(1)==-1||index(2)==-1)%没有找到
                            ...
                            p(num)=p(num)*tmp; %基于数据点与观测数据之间的关系tmp更新概率p
