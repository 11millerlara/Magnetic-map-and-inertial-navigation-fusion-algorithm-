function [pos]=EKPF2(n,y,x,sl,deg,p,q,mag,dataset, min_lat ,min_lon)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        Q=Q(1:2,1:2);
        A=A(1:2,1:2);
        Y=KNN(x_e,y_e,mag(i,:),dataset, min_lat ,min_lon);
        Y=Y.';
        %% 预测步
        Xminus=A*X;
        Pminus=A*Pplus*A.'+Q;
...
%         Pplus=(eye(4)-K*C)*Pminus;
        ...
        for j=1:n
            ...
            index=Find3(dataset,particle(j,2),particle(j,1), min_lat ,min_lon);
            if isnan(index(1))
                index(1) = -1;
                index(2) = -1;
            end
...
function [result]=KNN(x,y,mag,dataset, min_lat ,min_lon)
    %Nnum=100;
    sizedataset = size(mag,2);
%     pos=zeros(Nnum,2);
    M=size(dataset,1);
    ...
    for i=1:M
        for j=1:N
            tmp(2) = (i-1)/10 +min_lat;
            tmp(1) = (j-1)/10 +min_lon;
